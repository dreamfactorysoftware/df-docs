// Screenshot pipeline for df-docs — captures the live admin UI into .shots/
// for review. Nothing overwrites static/img/ automatically.
//
//   npm i -D playwright && npx playwright install chromium
//   DF_EMAIL=admin@example.com DF_PASS=secret node scripts/shots.mjs
//
// Optional: DF_URL (defaults to local df-prod). Dialog-level shots need
// per-page scripts; add those when a page is rewritten.
...
const hideBanner = () => page.evaluate(() => {
  ...
  // matches the app root and hides the whole UI.
