...
| **API Discovery** | Automatic — all database and file services are detected at connection time |
...
:::tip No API selection needed
You no longer need to select a specific database service. The MCP server **automatically discovers all database and file storage services** available in your DreamFactory instance when an AI client connects. This means a single MCP server gives your AI assistant access to every configured service.
:::
