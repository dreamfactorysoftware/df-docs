---
sidebar_position: 1
title: DreamFactory Platform Features & API Management Overview
id: introducing-rest-and-dreamfactory
description: DreamFactory generates governed REST and GraphQL APIs for any data source — databases, files, IoT — with RBAC, SSO, and zero backend code.
keywords: [DreamFactory platform, DreamFactory features, DreamFactory api management platform features, governed API access, REST API, GraphQL API, API management, enterprise API platform, self-hosted API, DreamFactory overview, dreamfactory api]
difficulty: beginner
---

# DreamFactory Platform Features & API Management Overview

## What is DreamFactory?

DreamFactory is a self-hosted platform that generates governed REST and GraphQL APIs for any data source — databases, file storage, email, IoT, and legacy systems — without writing backend code. Rather than hand-coding API layers for every data source your organization needs to expose, DreamFactory introspects your data source schema and instantly generates a fully documented, secured API with standard HTTP endpoints for all CRUD operations.

The platform's governed access model is central to enterprise adoption. Every API generated by DreamFactory is secured by default: clients must authenticate via an API key before any endpoint is accessible, and that API key is bound to a role-based access control (RBAC) policy that restricts which endpoints, HTTP methods, and data fields the caller can interact with. Beyond API key binding, DreamFactory supports identity passthrough — the authenticated user's identity is forwarded to the downstream data source so that database-level audit logs reflect the real user, not a generic service account. Rate limiting, SSO integration (LDAP, SAML, OAuth), and ELK-stack audit logging complete the governance picture.

## Key Platform Features

| Feature | Description | Docs |
|---|---|---|
| Auto-generated REST & GraphQL APIs | Instantly generate fully documented REST and GraphQL APIs for 20+ database types — no backend code required | [Database APIs](/api-generation-and-connections/api-types/database/generating-a-database-backed-api) |
| Role-Based Access Control (RBAC) | Restrict access per endpoint, HTTP method, table, and field — create read-only, write-only, or fully scoped roles | [Role-Based Access](/Security/role-based-access) |
| API Key Management | Generate and scope API keys per role; rotate keys without changing application code | [API Keys](/api-generation-and-connections/api-keys) |
| Event Scripts | Attach PHP, Python, or Node.js scripts to any API event for request validation, response transformation, and business logic enforcement | [Event Scripts](/api-generation-and-connections/event-scripts) |
| SSO & Identity Passthrough | Integrate LDAP, Active Directory, SAML 2.0, OAuth 2.0, and OpenID Connect; user identity passes through to the data source for native audit logging | [SSO & Authentication](/Security/authenticating-your-apis) |
| Audit Logging via ELK | Ship all API activity to Logstash/Elasticsearch for real-time dashboards of API key usage, HTTP status codes, and request patterns | [Logstash Integration](https://blog.dreamfactory.com) |
| Interactive OpenAPI Docs | Auto-generated Swagger/OpenAPI documentation for every generated API, with a live "try it" interface | [Interacting with APIs](/api-generation-and-connections/interacting-with-the-api) |
| Self-Hosted Deployment | Deploy on Linux, Windows Server, Docker, Kubernetes/Helm, or Raspberry Pi — no vendor lock-in, no data leaves your infrastructure | [Installation Guides](/getting-started/installing-dreamfactory/docker-installation) |

## DreamFactory vs. Hand-Coded APIs

| Feature | Hand-Coded API | DreamFactory |
|---|---|---|
| Initial setup time | Weeks to months per data source | Minutes — schema introspection is automatic |
| Role-based access control | Must be designed and coded from scratch | Built-in RBAC UI, applied per endpoint and HTTP method |
| API documentation | Requires manual OpenAPI authoring | Auto-generated Swagger docs for every endpoint |
| Multi-database support | Separate codebase per database type | Single platform supports 20+ databases via connectors |
| Maintenance overhead | Every schema change requires code updates and redeployment | Schema changes are reflected automatically on next introspection |
| Security defaults | Depends on developer discipline | All endpoints private by default; API key required |

## Supported Data Sources

DreamFactory supports a broad range of connectors across three categories:

**Database Connectors**
MySQL, PostgreSQL, Microsoft SQL Server, Oracle, MongoDB, SQLite, AlloyDB, MariaDB, Cassandra, DynamoDB, IBM Db2, HANA, Snowflake, and more. See the [database overview](/api-generation-and-connections/api-types/database/database-overview) for the full list.

**File & Storage Connectors**
Amazon S3, Azure Blob Storage, SFTP/FTP servers, local filesystem, and Google Cloud Storage. DreamFactory exposes file connectors as REST APIs supporting upload, download, directory listing, and deletion operations. Learn more in the [file services overview](/api-generation-and-connections/api-types/file-services/file-services-overview).

**Messaging, Email & Cache Connectors**
SMTP email providers (SendGrid, Mandrill, SMTP relay), push notification services (APNS, GCM), Redis and Memcached caching layers, and SOAP-to-REST conversion for legacy enterprise web services.
